...
import { IonAccordionGroup, IonAccordion, IonItem, IonLabel, IonList, IonListHeader, } from '@ionic/angular/standalone';
      ...
      <ion-list-header>
        Pogoda w twojej lokalizacji
            ...
            <ion-item>
             ...
             <ion-item>
              <ion-label>
                Warunki: {{weather()?.conditions}}
              </ion-label>
            </ion-item>
      </ion-list>
          } @else {
            <ion-item [button]="true" [detail]="true" (click)="weatherService.state.get(geolocationService.state.position()!)">
            <ion-label>
              Pobierz pogodę
            </ion-label>
            </ion-item>
...
export class WeatherComponent {
  ...
  weather = computed(
    () => this.weatherService.state.weather()?.days[0]
  )
}
